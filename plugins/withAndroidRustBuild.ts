...
const withAndroidRustBuild: ConfigPlugin<AndroidRustBuildPluginProps> = (config, props) => {
	config = withAndroidManifest(config, async config => {
		...
		let application: any[] = manifest.application || []
...
export interface AndroidRustBuildPluginProps extends CloneRepoPluginProps {
	crateName: string
	libName: string
	targets: string[]
}
...
async function buildRustForAndroid(props: AndroidRustBuildPluginProps, config: ExportedConfigWithProps<unknown>) {
	...
	fs.mkdirSync(androidProjectDir, { recursive: true })
