...
export type BadgeVariant = "info" | "destructive"

export type BadgeProps = Omit<TextProps, "variant"> & {
	/**
 ...
 */
export const Badge = memo(({ variant = "destructive", className, textVariant = "caption2", children, maxCount, ...props }: BadgeProps) => {
	return (
		<View
			style={BORDER_CURVE}
			className={cn(
				...
				!children ? "ios:-right-px -right-0.5 top-0 h-2.5 w-2.5" : "ios:-right-1 -right-1.5 -top-0.5 min-w-4 px-1"
			)}
		>
			<Text
				variant={textVariant}
...
export function getCount(count: React.ReactNode, maxCount?: number) {
	if (maxCount == null) {
		return count
	}
