...
export const InputPrompt = memo(() => {
	...
	const [state, setState] = useState<InputPromptParams>({
		id: "none",
		title: translateMemoized("inputPrompt.defaults.title")
	})
	...
	const buttons = useMemo((): (Omit<AlertButton, "onPress"> & {
		...
		return [
			{
				text: translateMemoized("inputPrompt.defaults.cancelText"),
				style: "cancel",
				onPress: () => {
					events.emit("inputPrompt", {
						type: "response",
			...
			{
				text: translateMemoized("inputPrompt.defaults.okText"),
				onPress: text => {
					events.emit("inputPrompt", {
						type: "response",
						data: {
