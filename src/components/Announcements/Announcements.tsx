...
const Announcement = memo(({ announcement, index }: { announcement: CFGAnnouncement; index: number }) => {
	...
	const lang = useLang()
	const [acknowledgedAnnouncements, setAcknowledgedAnnouncements] = useMMKVObject<Record<string, boolean>>(
...
const Announcements = memo(({ cfg }: { cfg: ICFG }) => {
	...
	const announcements = useMemo(() => {
		return cfg.announcements.filter(
			announcement =>
				announcement.active &&
				(announcement.platforms.includes("mobile") ||
					(announcement.platforms.includes("all") &&
						typeof (acknowledgedAnnouncements || {})[announcement.uuid] == "undefined"))
		)
