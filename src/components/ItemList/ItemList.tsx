...
export const ItemList = memo(
	({ navigation, route, items, searchTerm, populateList, loadDone, listDimensions }: ItemListProps) => {
		...
		const getThumbnail = useCallback((item: Item) => {
			if (
				item.type == "file" &&
				canCompressThumbnail(getFileExt(item.name)) &&
		...
		const estimatedItemSize = useMemo(() => {
			...
			const gridLengthDefault: number =
				Math.floor((dimensions.width - (insets.left + insets.right)) / itemsPerRow) + 55
